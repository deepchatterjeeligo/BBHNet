...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        self, channels, kernel_size, dropout=0, activation=nn.ReLU, **kwargs
    ):
        super().__init__()
        self.channels = channels
        self.activation = activation()
        self.conv = nn.Conv1d(
            channels, channels, kernel_size, padding=kernel_size // 2, **kwargs
        )
        self.conv_one_cross_one = nn.Conv1d(channels, channels, 1, **kwargs)
...
class CausalConv1d(nn.Conv1d):
    ...
    def __init__(self, *args, **kwargs):
        ...
        (
            kernel_size,
            *_,
        ) = self.kernel_size  # assuming only one entry in kernel_size
...
class TCN(nn.Module):
    def __init__(
        self, channels=64, kernel_size=3, dilations=(1, 2, 4, 8), dropout=0.0
    ):
        super().__init__()
